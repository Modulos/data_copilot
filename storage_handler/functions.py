...
from storage_handler.base import ClientABC
...
def _get_client(func):
    ...
    def wrapper(uri, *args, **kwargs):
        if "dfs.core.windows.net" in uri:
            from storage_handler.azure_client import AzureClient

            client = AzureClient(uri)
        elif uri.startswith("volume://"):
            from storage_handler.localstorage_client import LocalStorageClient
